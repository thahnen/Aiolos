...
namespace GLCM {
     ...
     *  REVIEW: Use when Mat-Type is not known by compile time -> usage at runtime!
     */
    unsigned int main_angle_range(const cv::Mat &image, Implementation impl, Range range, unsigned int max_r) {
    ...
    DLL std::vector<unsigned int> main_angles_range(const cv::Mat &image, Implementation impl, Method meth,
                                                    Range range, unsigned int max_r) {
        unsigned int max_radius = max_r != 0 ? max_r : ceil(sqrt(2)*std::max(image.cols/2, image.rows/2));
        ...
        switch (meth) {
            ...
            default:
                ...
                if (meth == MEDIAN) {
                    ...
                    value = getQuantileValue(orientation_dist, Quantil::LOWER_QUARTILE);
                } else {
        ...
        unsigned int main_angle(const cv::Mat& image, Implementation impl, unsigned int max_r) {
            ...
            switch (image.type() & CV_MAT_DEPTH_MASK) {
                ...
                case CV_32SC1:
                    ...
                    break;
                default:
                    throw std::runtime_error("[GLCM::CT::main_angle] Unsupported Mat-type!");
            }
